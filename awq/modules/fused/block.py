...
class LlamaLikeBlock(nn.Module):
    ...
    def __init__(self, hidden_size, n_heads, n_kv_heads, qkv_layer, o_proj,
        ...
        self.attn = QuantAttentionFused(self.hidden_size,
                                        self.n_heads,
                                        self.n_kv_heads,
                                        qkv_layer,
                                        o_proj,
...
class MPTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, n_heads, qkv_layer, o_proj, mpt_mlp,
                 norm_1, norm_2, dev, max_seq_len):
        super().__init__()
        self.n_heads = n_heads
        self.n_kv_heads = 0
        self.hidden_size = hidden_size
        self.norm_1 = norm_1
        self.attn = QuantAttentionFused(hidden_size,
                                        self.n_heads,
                                        self.n_kv_heads,
                                        qkv_layer,
                                        o_proj,
...
class FalconDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = QuantAttentionFused(
            ...
            max_seq_len=max_seq_len,
            use_alibi=False,
            attention_shapes=attention_shapes).to(dev)
