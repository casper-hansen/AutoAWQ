...
class RoPE(nn.Module):
    ...
    def __init__(self, hidden_size, n_heads, max_seq_len, device, base=10000):
        ...
        self.freqs_cis = nn.Parameter(
            self.precompute_freqs_cis(hidden_size // n_heads,
                                      max_seq_len * 2,
                                      theta=self.base).to(device),
            requires_grad=False)
    ...
    def forward(self, xq: torch.Tensor, xk: torch.Tensor, start_pos: int,
        ...
        xq_out = torch.view_as_real(xq_ * freqs_cis).transpose(-2,
                                                               -1).flatten(3)
        xk_out = torch.view_as_real(xk_ * freqs_cis).transpose(-2,
                                                               -1).flatten(3)
...
class ALiBi(nn.Module):
    ...
    def __init__(self, n_heads, max_seq_len, device, alibi_bias_max=8):
        ...
        slopes, bias = self.build_alibi_bias(n_heads,
                                             max_seq_len,
                                             alibi_bias_max=alibi_bias_max)
        self.slopes = nn.Parameter(slopes.float().to(device),
                                   requires_grad=False)
    ...
    def build_alibi_bias(n_heads,
                         ...
                         alibi_bias_max=8,
                         dtype=torch.float32):
        alibi_bias = torch.arange(1 - seq_len, 1,
                                  dtype=torch.int32).view(1, 1, 1, seq_len)
...
class QuantAttentionFused(nn.Module):
    ...
    def __init__(self,
        ...
        self.rope_theta = rope_theta  # for not default rope

        # attention shapes for self attention
        self.attention_shapes = get_attention_shapes(attention_shapes,
                                                     max_seq_len,
                                                     self.cache_batch_size,
                                                     n_heads, n_kv_heads,
                                                     self.head_dim)
        # cache store that rolls cache
        self.cache = WindowedCache(self.attention_shapes["cache_v"],
                                   self.attention_shapes["cache_k"],
                                   self.max_seq_len, dev)
    ...
    def forward(self,
        ...
        if self.is_hf_transformers and "past_key_value" in kwargs and kwargs[
        ...
        if seqlen > 1 or not FT_INSTALLED:
            ...
            keys_store = (
                xk.reshape((bsz, seqlen) +
                           self.attention_shapes["xk_reshape"]).permute(
                               0, 2, 3, 1, 4).contiguous())

            self.cache.update_kv(values_store, keys_store, bsz, self.start_pos,
                                 seqlen)
            ...
            if self.n_kv_groups != 0:
                keys = torch.repeat_interleave(keys,
                                               dim=2,
                                               repeats=self.n_kv_groups)
            ...
            if FLASH_INSTALLED and not self.use_alibi:
                ...
                output = torch.matmul(
                    scores, values)  # (bs, n_local_heads, slen, head_dim)
                attention_weight = output.transpose(1, 2).contiguous().view(
                    bsz, seqlen, -1)
        else:
